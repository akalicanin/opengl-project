...
int main() {
	...
	glm::vec3 cubePositions[] = {
		glm::vec3(-2.0f, -1.3f, 0.5f),
		glm::vec3(2.0f, 1.3f, 0.5f),
		glm::vec3(1.4f, 3.2f, 1.6f),
		glm::vec3(0.4f, -0.6f, -4.4f),
		glm::vec3(1.9f, -4.0f, -3.0f),
		glm::vec3(3.3f, 3.3f, 3.3f),
		glm::vec3(-3.3f, -3.3f, -3.3f),
		glm::vec3(0.1f, 6.0f, -5.0f),
	...
	*/
	...
	glm::vec3 pointLightPositions[] = {
		glm::vec3(0.0f,	5.0f, -2.8f),
		glm::vec3(5.0f, 3.0f, 3.0f),
		glm::vec3(1.0f, -2.0f, 0.4f)
	};
	*/
	...
	// Ucitavanje tekstura
	uint boxDiffuse, boxSpecular, boxHeight, csboxDiffSpec;
	uint dnkGreenDiff, dnkRedDiff, dnkSpec;
	boxDiffuse = loadTexture("textures/dosadnakutija.png");
	boxSpecular = loadTexture("textures/dosadnakutijaSPEC.png");
	csboxDiffSpec = loadTexture("textures/csbox.png");
	...
	Model gltfTorus("models/torusGLTF/torusCone.glb");

	Model torusModel("models/torus/torus.obj");

	Model torusConeModel("models/toruscone/torus.obj");

	Model backpack("models/backpackGL/voodoo obojen.glb");
	...
	while (!glfwWindowShouldClose(window)) {
		...
		// point light kruzni
		cubeShader->setVec3("pointLights[28].position", glm::vec3(viewMatrix * glm::vec4(lightcubePos, 1.0f)));
		cubeShader->setVec3("pointLights[28].ambient", 0.05f, 0.05f, 0.05f);
		cubeShader->setVec3("pointLights[28].diffuse", lightColor);
		cubeShader->setVec3("pointLights[28].specular", 1.0f, 1.0f, 1.0f);
		cubeShader->setFloat("pointLights[28].constant", 1.0f);
		cubeShader->setFloat("pointLights[28].linear", 0.09f);
		cubeShader->setFloat("pointLights[28].quadratic", 0.032f);
		// spotLight
		...
		cubeShader->use();
		modelMatrix = glm::translate(glm::mat4(1.0f), glm::vec3(-10.0f, 2.0f, 0.0f));
		cubeShader->setMat4("model", modelMatrix);
		torusModel.draw(*cubeShader);

		modelMatrix = glm::translate(glm::mat4(1.0f), glm::vec3(-10.0f, 2.0f, 8.0f));
		cubeShader->setMat4("model", modelMatrix);
		torusConeModel.draw(*cubeShader);


		modelMatrix = glm::translate(glm::mat4(1.0f), glm::vec3(-10.0f, 12.0f, 8.0f));
		cubeShader->setMat4("model", modelMatrix);
		gltfTorus.draw(*cubeShader);

		modelMatrix = glm::translate(glm::mat4(1.0f), glm::vec3(-15.0f, 2.0f, 0.0f));
		cubeShader->setMat4("model", modelMatrix);
